...
В UNIX хорошая практика — проверять **все, что не является пользовательскими данными**.

# Как выполняется проверка? (Хэш-суммы и база знаний)

Идея очень простая, но гениальная:
1. Создание эталонной базы.
...
4. Если это **взлом**:
   ...
   - Не доверять утилитам на этом сервере.
   - Сравнить с эталоном (который хранится вне сервера).
   - Самый надежный путь: переустановка системы с нуля и восстановление из резервных копий.
5. **Если это легитимное изменение**:
   - Зафиксировать новую базу.
   - Занести изменение в лог и систему контроля версий.
